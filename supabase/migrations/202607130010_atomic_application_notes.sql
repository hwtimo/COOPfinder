-- Atomically update normalized application notes and append a private event.
...
begin
  ...
  select nullif(pg_catalog.btrim(application.notes), '')
...
comment on function public.update_application_notes(uuid, text) is
  'Atomically stores normalized caller-owned application notes and appends one content-free note_updated event. Normalized no-op requests return unchanged without writing.';
