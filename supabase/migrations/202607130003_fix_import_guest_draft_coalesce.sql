-- Correct invalid schema qualification of the COALESCE SQL expression.
...
begin
  ...
  loop
      ...
      where e.user_id = v_user_id
        ...
        and pg_catalog.lower(pg_catalog.regexp_replace(pg_catalog.btrim(e.entry_text), '[[:space:]]+', ' ', 'g'))
          = pg_catalog.lower(pg_catalog.regexp_replace(
              pg_catalog.btrim(coalesce(pg_catalog.nullif(v_text, ''), v_title)),
              '[[:space:]]+', ' ', 'g'
            ))
    ) then
    ...
    insert into public.master_profile_entries (
      ...
      pg_catalog.nullif(v_title, ''),
      coalesce(pg_catalog.nullif(v_text, ''), v_title),
      v_entry_skills,
      true,
      coalesce((
