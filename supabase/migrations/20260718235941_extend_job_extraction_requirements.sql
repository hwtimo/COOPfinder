-- Extend atomic extraction persistence with optional, strictly validated
-- structured requirements while preserving the legacy job-extraction-v1 shape.
...
begin
  ...
  if p_extracted ? 'structuredRequirements' then
    v_structured := p_extracted->'structuredRequirements';

    if pg_catalog.jsonb_typeof(v_structured) is distinct from 'object'
      or not (v_structured ?& array[
        'requiredSkills',
        'preferredSkills',
      ...
      or exists (
        select 1
        from pg_catalog.jsonb_object_keys(v_structured) as structured_key(key)
        where structured_key.key <> all (array[
          'requiredSkills',
          'preferredSkills',
          'requiredTechnologies',
          'preferredTechnologies',
    ...
    foreach v_field in array array[
      ...
      v_field_json := v_structured->v_field;
      if pg_catalog.jsonb_typeof(v_field_json) is distinct from 'array' then
        return query select 'invalid_input'::text, null::uuid;
        return;
      end if;
      ...
      if v_field in (
        ...
        v_max_items := 60;
        v_max_length := 80;
      elsif v_field = 'responsibilities' then
        v_max_items := 50;
        v_max_length := 1000;
      else
        v_max_items := 80;
      ...
      if pg_catalog.jsonb_array_length(v_field_json) > v_max_items then
        return query select 'invalid_input'::text, null::uuid;
        return;
      end if;
      ...
      for v_item in
        select item.value
        from pg_catalog.jsonb_array_elements(v_field_json) as item(value)
      loop
        if pg_catalog.jsonb_typeof(v_item) is distinct from 'string' then
          return query select 'invalid_input'::text, null::uuid;
          return;
...
comment on function public.persist_job_extraction(uuid, jsonb, numeric) is
  'Atomically stores one validated legacy or structured job-extraction-v1 result for a caller-owned pasted-text job and records one minimal success event.';
