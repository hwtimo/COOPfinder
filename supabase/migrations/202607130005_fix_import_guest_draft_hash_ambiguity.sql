-- Qualify the guest import ledger hash predicate to avoid output-column ambiguity.
...
begin
  ...
  if exists (
    select 1 from public.guest_draft_imports gdi
    where user_id = v_user_id and gdi.draft_hash = v_hash
  ) then
